...
test.describe("検索ページ", () => {
  ...
  test("タイトルが「検索 - CaX」となること", async ({ page }) => {
    await page.setViewportSize({ width: 1920, height: 1080 });
    await page.goto("/search");
    await expect(page).toHaveTitle("検索 - CaX", { timeout: 3_000 });
  });
  ...
  test("検索結果が表示される", async ({ page }) => {
    ...
    await expect(page.locator("main article").first()).toBeVisible({ timeout: 3_000 });
  ...
  test("キーワードと since:YYYY-MM-DD を組み合わせて検索できること", async ({ page }) => {
    ...
    await page.waitForURL(/\/search\?q=/, { timeout: 3_000, waitUntil: "commit" });
